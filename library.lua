...
do
	...
	function utility:Tween(obj, prop, val, time, direction, style)
		if self:HasProperty(obj, prop) then
			...
			tween.Connection = self:Connection(runservice.RenderStepped, function(dt)
				...
				pcall(function()
					local progress =
						tweenService:GetValue(a, style or Enum.EasingStyle.Linear, direction or Enum.EasingDirection.In)
					local newVal
			...
			printconsole(
				"unable to tween: invalid property " .. tostring(prop) .. " for object " .. tostring(obj),
				255,
				0,
				0
			)
	...
	function utility:Draw(class, properties)
		...
		function drawing:Update()
			...
			if parent ~= nil then
				parentSize = (parent.Class == "Square" or parent.Class == "Image") and parent.Object.Size
			...
			if drawing.Class == "Square" or drawing.Class == "Image" then
				drawing.Object.Size = typeof(drawing.Size) == "Vector2" and drawing.Size
					or typeof(drawing.Size) == "UDim2" and utility:UDim2ToVector2(drawing.Size, parentSize)
			end
			...
			then
				drawing.Object.Position = parentPos
					+ (
						typeof(drawing.Position) == "Vector2" and drawing.Position
			...
			if not table.find(blacklistedProperties, i) then
				...
				if i == "Size" and (class == "Square" or class == "Image") then
					drawing.Object.Size = utility:UDim2ToVector2(
						v,
						drawing.Parent == nil and workspace.CurrentCamera.ViewportSize or drawing.Parent.Object.Size
					)
				...
				elseif i == "Position" and (class == "Square" or class == "Image" or class == "Text") then
					drawing.Object.Position = utility:UDim2ToVector2(
						v,
						drawing.Parent == nil and newVector2(0, 0) or drawing.Parent.Object.Position
					)
				...
				if
					table.find({ "ThemeColor", "OutlineThemeColor", "ThemeColorOffset", "OutlineThemeColorOffset" }, i)
					and lastval ~= v
				then
...
function library:init()
				...
				if option ~= nil then
					...
					elseif option.class == "bind" then
						option:SetBind(
							value == nil and "none"
								or (
									utility:HasProperty(Enum.KeyCode, value) and Enum.KeyCode[value]
	...
	function self:SaveConfig(name)
		...
		local s, e = pcall(function()
			...
			writefile(
				self.cheatname .. "/" .. self.gamename .. "/configs/" .. name .. self.fileext,
				http:JSONEncode(cfg)
			)
		end)
	end
			...
			if library.open then
				...
				if library.CurrentTooltip ~= nil then
					...
					tooltipObjects.background.Size = UDim2.new(
						0,
						tooltipObjects.text.TextBounds.X + 6 + (library.CurrentTooltip.risky and 60 or 0),
						0,
		...
		for _, v in next, library.drawings do
			...
			if v.ThemeColorOutline and library.theme[v.ThemeColorOutline] then
				v.Object.OutlineColor = utility:AddRGB(
					...
					fromrgb(v.OutlineThemeColorOffset, v.OutlineThemeColorOffset, v.OutlineThemeColorOffset)
				)
			end
		end
	end
	...
	function self:SendNotification(message, time, color)
		...
		task.spawn(function()
			...
			utility:Connection(
				utility:Tween(notification.background, "Position", newUDim2(0, -500, 0, 0), 0.25).Completed,
				function()
					notification:Remove()
				end
			)
	...
	function self.NewWindow(data)
										...
										for idx, val in next, currentList.values do
											...
											if valueObj then
												valueObj.background.Transparency = (
													typeof(newSelected) == "table" and table.find(newSelected, val)
													or newSelected == val
												)
														and 1
													or 0
		...
		local function tooltip(option)
			utility:Connection(option.objects.holder.MouseEnter, function()
				tooltipObjects.background.Visible = (not (option.tooltip == "" or option.tooltip == nil)) and true
					or false
			...
			function tab:AddSection(text, side, order)
				...
				function section:AddToggle(data)
					...
					function toggle:SetState(bool, nocallback)
						if typeof(bool) == "boolean" then
							...
							self.objects.border1.ThemeColor = bool and "Accent"
								or (self.objects.holder.Hover and "Accent" or "Option Border 1")
							self.objects.text.ThemeColor = bool
									and (self.risky and "Risky Text Enabled" or "Option Text 1")
								or (self.risky and "Risky Text" or "Option Text 3")
							...
							if option.enabled then
								if option.class == "color" or option.class == "bind" then
									option.objects.holder.Position =
										newUDim2(1, -option.objects.holder.Object.Size.X - x, 0, 0)
									x = x + option.objects.holder.Object.Size.X
								elseif option.class == "slider" or option.class == "list" then
									option.objects.holder.Position =
										newUDim2(0, 0, 1, -option.objects.holder.Object.Size.Y - y)
					...
					function toggle:AddBind(data)
						...
						if bind.bind == "none" then
							...
							bind.indicatorValue:SetKey(
								(bind.text == nil or bind.text == "") and (bind.flag == nil and "unknown" or bind.flag)
									or bind.text
							) -- this is so dumb
						...
						function bind:SetBind(keybind)
							...
							self.indicatorValue:SetKey(
								(self.text == nil or self.text == "") and (self.flag == nil and "unknown" or self.flag)
									or self.text
							) -- this is so dumb
						...
						utility:Connection(inputservice.InputBegan, function(inp)
							...
							elseif bind.binding then
								local key = (
									...
									}, inp.UserInputType) and not bind.nomouse
							...
							elseif not bind.binding and self.bind == "none" then
								...
								bind.indicatorValue:SetEnabled(display and not bind.noindicator)
							elseif
								(inp.KeyCode == bind.bind or inp.UserInputType == bind.bind) and not bind.binding
							then
					...
					function toggle:AddSlider(data)
						...
						for i, v in next, data do
							if
								not table.find(blacklist, i) and (slider[i] ~= nil and typeof(slider[i]) == typeof(v))
							then
								slider[i] = v
							end
						end
							...
							utility:Connection(slider.objects.holder.MouseButton1Down, function()
								if inputservice:IsKeyDown(Enum.KeyCode.LeftControl) then
									if slider.focused then
										...
										objs.text.Text = tostring(slider.value)
										...
										c = utility:Connection(inputservice.InputBegan, function(inp)
											if library.numberStrings[inp.KeyCode.Name] then
												...
												objs.text.Text = string.format("%.14g", slider.value)
													.. tostring(slider.suffix)
													.. "/"
													.. slider.max
													.. tostring(slider.suffix)
						...
						function slider:SetValue(value, nocallback)
							if typeof(value) == "number" then
								local newValue =
									clamp(self.increment * floor(value / self.increment), self.min, self.max)
								...
								self.objects.text.Text = string.format("%.14g", newValue)
									.. tostring(self.suffix)
									.. "/"
									.. self.max
									.. tostring(self.suffix)
								self.objects.text.ThemeColor = (self.min < 0 and newValue == 0 or newValue == self.min)
										and (self.risky and "Risky Text" or "Option Text 3")
									or (self.risky and "Risky Text Enabled" or "Option Text 1")
						...
						function list:Select(option, nocallback)
							...
							if option ~= nil then
								...
								local text = typeof(option) == "table"
										and (#option == 0 and "none" or table.concat(option, ", "))
									or tostring(option)
				...
				function section:AddSlider(data)
					...
					do
						...
						utility:Connection(slider.objects.plusDetector.MouseButton1Down, function()
							slider:SetValue(
								slider.value
									+ (inputservice:IsKeyDown(Enum.KeyCode.LeftShift) and 10 or slider.increment)
							)
						end)
								...
								if slider.focused then
									...
									c = utility:Connection(inputservice.InputBegan, function(inp)
										if library.numberStrings[inp.KeyCode.Name] then
											...
											objs.text.Text = slider.text
												.. ": "
												.. string.format("%.14g", slider.value)
												.. tostring(slider.suffix)
												.. " ["
												.. inputNumber
												.. "]"
				...
				function section:AddSeparator(data)
					...
					for i, v in next, data do
						if
							...
							and (separator[i] ~= nil and typeof(separator[i]) == typeof(v))
						then
							separator[i] = v
						end
					end
				...
				function section:AddBox(data)
					...
					function box:CaptureFocus(clear)
						...
						c = utility:Connection(inputservice.InputBegan, function(inp)
							...
							elseif
								#inp.KeyCode.Name == 1
								or table.find(whitelistedBoxKeys, inp.KeyCode)
							...
							then
								...
								local keyString = inp.KeyCode.Name == "Space" and " "
								...
								if
									not (
										...
										and not inputservice:IsKeyDown(Enum.KeyCode.RightShift)
									)
								then
	...
	do
		...
		function self.watermark:Update()
			...
			if library.flags.watermark_enabled then
				...
				self.position = (
					self.lock == "Top Right" and newUDim2(0, screensize.X - size.X - 15, 0, 15)
					...
					or self.lock == "Bottom Left" and newUDim2(0, 15, 0, screensize.Y - size.Y - 15)
					or self.lock == "Top" and newUDim2(0, screensize.X / 2 - size.X / 2, 0, 15)
					or newUDim2(library.flags.watermark_x / 100, 0, library.flags.watermark_y / 100, 0)
				)
...
function library:CreateSettingsTab(menu)
	...
	mainSection:AddToggle({
		text = "Disable Movement If Open",
		flag = "disablemenumovement",
	...
	mainSection:AddSlider({
		...
		callback = function()
			library.keyIndicator:SetPosition(
				newUDim2(library.flags.keybind_indicator_x / 100, 0, library.flags.keybind_indicator_y / 100, 0)
			)
		end,
	})
	mainSection:AddSlider({
	...
	mainSection:AddList({
		...
		flag = "watermark_pos",
		selected = "Custom",
		values = { "Top", "Top Left", "Top Right", "Bottom Left", "Bottom Right", "Custom" },
